...
contract Unit is Fixture {
        ...
        returns (IVeloDeployFactory.PoolAddresses memory addresses)
    {
        ...
        IAmmModule.AmmPosition memory ammPosition = core
            .ammModule()
            .getAmmPosition(postition.ammPositionIds[0]);
        ...
        assertEq(
            ammPosition.tickUpper - ammPosition.tickLower,
            deployParams.width
        );
        assertTrue(ammPosition.liquidity > 0);

        IPulseStrategyModule.StrategyParams memory strategyParams = abi.decode(
            postition.strategyParams,
            (IPulseStrategyModule.StrategyParams)
        );
        assertEq(
            uint256(strategyParams.strategyType),
            uint256(deployParams.strategyType)
        );
        assertEq(
            strategyParams.tickNeighborhood,
            deployParams.tickNeighborhood
        );
        ...
        assertEq(
            ...
            deployParams.maxLiquidityRatioDeviationX96
        );

        IVeloAmmModule.CallbackParams memory callbackParams = abi.decode(
            postition.callbackParams,
