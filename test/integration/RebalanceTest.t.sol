...
contract IntegrationTest is Test, DeployScript {
    ...
    function setUp() external {
        ...
        params.strategyParams = IPulseStrategyModule.StrategyParams({
            ...
            width: 1000, // Width of the interval
    ...
    function testRebalancePulseTamperPulse() external {
        ...
        deal(Constants.BASE_ZRO, user, wstethAmount);

        IERC20(Constants.BASE_WETH).safeIncreaseAllowance(address(wstethWeth1Wrapper), wethAmount);
        IERC20(Constants.BASE_ZRO).safeIncreaseAllowance(address(wstethWeth1Wrapper), wstethAmount);
