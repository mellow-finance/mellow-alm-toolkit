...
contract RebalancerTest is Fixture {
    ...
    function testRevertHighSlippage() external {
        ...
        TestRebalancingBot bot = new TestRebalancingBot(
        ...
        uint256 increaseLiquidityD9 =
            getMaxIncreaseLiquidityD9(rebalanceData.target, rebalanceData.info);
        /**
        ...
        rebalancer.rebalance(
            ICore.RebalanceParams({
                ...
                data: abi.encode(increaseLiquidityD9 - 1)
            })
        );
    }
    ...
    function testSwapRebalance() external {
        ...
        for (uint24 movePart = 2; movePart < 27; movePart++) {
            ...
            rebalanceData.target = estimateMinLiquidities(rebalanceData.target, rebalanceData.info);
            /**
             * Core reduces liquidity of a new position to the slippage tolerance,
            ...
            rebalancer.rebalance(
                ICore.RebalanceParams({
                    id: rebalanceData.target.id,
                    callback: address(bot),
                    data: abi.encode(increaseLiquidityD9)
                })
            );
    ...
    function testChangeStrategyLazyToLazy(int8 tickDelta) external {
        ...
        rebalancer.rebalance(
            ICore.RebalanceParams({
                ...
                data: abi.encode(increaseLiquidityD9 + 100)
            })
        );
        (, info) = rebalancer.managedPositionInfo(address(lpWrapper));
        ...
        for (uint256 i = 0; i < deltas.length; i++) {
            ...
            if (rebalanceData.isRebalanceRequired) {
                ...
                rebalanceData.target =
                    estimateMinLiquidities(rebalanceData.target, rebalanceData.info);
                rebalancer.rebalance(
                    ICore.RebalanceParams({
                        id: rebalanceData.target.id,
                        callback: address(bot),
    ...
    ) internal view returns (ICore.TargetPositionInfo memory) {
        uint160 sqrtPriceX96 = IAmmModule(contracts.ammModule).getSqrtPriceX96(input.pool);

        uint256 capitalInput;
        for (uint256 index = 0; index < input.ammPositionIds.length; index++) {
            (uint256 amount0, uint256 amount1) =
                IAmmModule(contracts.ammModule).tvl(input.ammPositionIds[index]);
    ...
    function estimateMovedCapitalD9(
        ...
        for (uint256 i = 0; i < input.ammPositionIds.length; i++) {
            (uint256 amount0, uint256 amount1) =
                IAmmModule(contracts.ammModule).tvl(input.ammPositionIds[i]);
        ...
        return Math.mulDiv(movedCapital, D9, capitalInput);
    }
}
...
contract TestRebalancingBot is IRebalanceCallback, Test {
    ...
    function call(
        ...
        uint256 increaseLiquidityD9 = abi.decode(data, (uint256));
        ...
        tokenIds = mintTarget(address(info.pool), increaseLiquidityD9, target);
