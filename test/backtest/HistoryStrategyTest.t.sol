...
contract HistoryTest is Test {
        ...
        external
    {
        ...
        if (amount0Delta > 0) {
            token0.safeTransferFrom(recipient, address(pool), uint256(amount0Delta));
        }
        if (amount1Delta > 0) {
            token1.safeTransferFrom(recipient, address(pool), uint256(amount1Delta));
        }
    }
    ...
    function _swapAmount(int256 amount0, int256 amount1) private returns (bool result) {
        ...
        result = true;
        try pool.swap(
            address(this), zeroForOne, amountSpecified, sqrtPriceLimitX96, abi.encode(address(this))
        ) {} catch {
            result = false;
        }
        return result;
    ...
    function _swapToPrice(uint160 sqrtPriceLimitX96) private returns (bool result) {
    ...
    function _mint(uint128 liquidity, int24 tickLower, int24 tickUpper)
        private
        returns (bool result)
    {
        result = true;
        try pool.mint(address(this), tickLower, tickUpper, liquidity, abi.encode(address(this))) {}
        catch {
            result = false;
        }
        return result;
    ...
    function _burn(uint128 liquidity, int24 tickLower, int24 tickUpper)
        private
        returns (bool result)
    {
        result = true;
        try pool.burn(tickLower, tickUpper, liquidity) {}
        catch {
            result = false;
        }
        return result;
    ...
    function poolTransaction(SwapTransaction[] memory transactions)
        public
        returns (uint256 successfulTransactions)
    {
    ...
    function _getFeeGrowthInside(int24 tickCurrent, int24 tickLower, int24 tickUpper)
        private
        view
        returns (uint256 feeGrowthInside0X128, uint256 feeGrowthInside1X128)
    {
        ...
        if (tickCurrent < tickLower) {
            ...
            uint256 delta0 = lowerFeeGrowthOutside0X128 + upperFeeGrowthOutside0X128;
            feeGrowthInside0X128 =
                delta0 > feeGrowthGlobal0X128 ? feeGrowthGlobal0X128 : feeGrowthGlobal0X128 - delta0;
            uint256 delta1 = lowerFeeGrowthOutside1X128 + upperFeeGrowthOutside1X128;
            feeGrowthInside1X128 =
                delta1 > feeGrowthGlobal1X128 ? feeGrowthGlobal1X128 : feeGrowthGlobal1X128 - delta1;
        } else {
    ...
    function _positionView() private {
        ...
        fee0 = feeGrowthInside0X128 > feeGrowthInside0LastX128
            ? Math.mulDiv(liquidity, feeGrowthInside0X128 - feeGrowthInside0LastX128, Q128)
            : 0;
        fee1 = feeGrowthInside1X128 > feeGrowthInside1LastX128
            ? Math.mulDiv(liquidity, feeGrowthInside1X128 - feeGrowthInside1LastX128, Q128)
            : 0;
        (totalValueInToken0Last, totalValueInToken1Last) = LiquidityAmounts.getAmountsForLiquidity(
            sqrtPriceX96,
            TickMath.getSqrtRatioAtTick(tickLower),
            TickMath.getSqrtRatioAtTick(tickUpper),
            liquidity
