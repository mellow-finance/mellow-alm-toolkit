...
/**
 * @title LazyPulseStrategyModule
 */
contract LazyPulseStrategyModule is IStrategyModule {
    error InvalidParams();
    error InvalidLength();
     ...
     * @notice throws InvalidParams if tick spacing is zero.
     */
    function validateStrategyParams(
        ...
        if (strategyParams.tickSpacing == 0) {
        ...
        returns (bool isRebalanceRequired, ICore.TargetNftsInfo memory target)
    {
        ...
        if (tick < tickLower) {
            ...
            int24 delta = -(tick % params.tickSpacing);
            if (delta > 0) delta -= params.tickSpacing;
            targetTickUpper = tick + delta;
            targetTickLower = targetTickLower - positionWidth;
        }
